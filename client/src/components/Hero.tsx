      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex flex-col lg:flex-row items-center">
          <div className="lg:w-1/2 lg:pr-12 mb-10 lg:mb-0 animate-slide-up">
            ...
            <p className="text-lg md:text-xl text-gray-600 dark:text-gray-300 mb-8 max-w-xl animate-fade-in" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="mt-2 flex items-center text-sm text-gray-500 dark:text-gray-400 animate-fade-in" style={{ animationDelay: '0.6s' }}>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl dark:shadow-gray-700 p-4 transform hover:scale-[1.02] transition-all duration-500">
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-2">
                <div className="flex overflow-x-auto gap-4 pb-4 scrollbar-hide">
                  {['To Do', 'In Progress', 'In Review', 'Done'].map((column, index) => (
                    <div key={index} className="flex-shrink-0 w-72 h-96 animate-scale-up" style={{ animationDelay: `${index * 0.1}s` }}>
                      <div className={`h-full rounded-lg p-4 ${
                        index === 0 ? 'bg-blue-50 dark:bg-blue-900/30' :
                        index === 1 ? 'bg-purple-50 dark:bg-purple-900/30' :
                        index === 2 ? 'bg-orange-50 dark:bg-orange-900/30' :
                        'bg-green-50 dark:bg-green-900/30'
                      }`}>
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="font-semibold text-gray-800 dark:text-gray-200">{column}</h3>
                          <span className="bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-medium px-2 py-1 rounded-full">
                            {index === 0 ? '3' : index === 1 ? '2' : index === 2 ? '1' : '4'}
                          </span>
                        </div>
                        ...
                        {[1, 2, 3].map((task, taskIndex) => (
                          <div 
                            ...
                            className={`bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm dark:shadow-gray-700 mb-3 transform hover:scale-[1.02] transition-all duration-300 ${
                              index === 1 && taskIndex === 0 ? 'border-l-4 border-[#1D9EE2]' : ''
                            } ${
                              index === 0 && taskIndex === 1 ? 'border-l-4 border-[#AB4A9C]' : ''
                            }`}
                          >
                            <div className="flex justify-between items-start mb-2">
                              <h4 className="font-medium text-sm text-gray-800 dark:text-gray-200">
                                {index === 0 && taskIndex === 0 ? 'Update homepage layout' :
                                 index === 0 && taskIndex === 1 ? 'Fix login button bug' :
                                 index === 1 && taskIndex === 0 ? 'Design new user flow' :
                                 index === 2 && taskIndex === 0 ? 'API integration' :
                                 index === 3 && taskIndex === 0 ? 'Write documentation' :
                                 `Task ${taskIndex + 1}`}
                              </h4>
                              <span className={`text-xs px-2 py-1 rounded-full ${
                                taskIndex === 0 ? 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200' :
                                taskIndex === 1 ? 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-800 dark:text-yellow-200' :
                                'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-200'
                              }`}>
                                {taskIndex === 0 ? 'High' : taskIndex === 1 ? 'Medium' : 'Low'}
                              </span>
                            </div>
                            <p className="text-gray-500 dark:text-gray-400 text-xs mb-3">
                              {index === 0 && taskIndex === 0 ? 'Update the homepage with new branding elements' :
                               index === 1 && taskIndex === 0 ? 'Create wireframes for new user onboarding experience' :
